	...
	class db_class extends db_connect{	
		...
		public function reset($kd){
			$stmt = $this->conn->prepare("UPDATE `tbl_asprak` SET pass_asprak = 'asistenpraktikum' WHERE `kode_asprak` = ?") or die($this->conn->error);
		...
		public function update($kode_asprak , $nama_asprak, $jk_asprak , $foto_asprak , $telp_asprak , $email_asprak){
			$stmt = $this->conn->prepare("UPDATE `tbl_asprak` SET `nama_asprak` = ? , `jk_asprak` = ? , `foto_asprak` = ? , `telp_asprak` = ? , `email_asprak` = ? WHERE `kode_asprak` = ?") or die($this->conn->error);
			$stmt->bind_param("ssssss", $nama_asprak, $jk_asprak , $foto_asprak , $telp_asprak , $email_asprak , $kode_asprak);
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
