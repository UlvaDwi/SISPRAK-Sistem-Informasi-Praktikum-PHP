	...
	class db_class extends db_connect{	
		...
		public function checkData($nama_lab, $kapasitas_lab){
			$stmt = $this->conn->prepare("select * from tbl_ruanglab");
			$gabung = $nama_lab.$kapasitas_lab;
			if ($stmt->execute()) {
				$result = $stmt->get_result();
				while ($i = $result->fetch_array()) {
					if ($i['nama_lab'].$i['kapasitas_lab']==$gabung) {
						return 1;
						break;
					}
				}
