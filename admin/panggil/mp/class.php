	...
	class db_class extends db_connect{	
		...
		public function checkData($nama_mp, $kode_jurusan, $kode_ta){
			$stmt = $this->conn->prepare("select * from tbl_matapraktikum");
			$gabung = strtolower(preg_replace('/\s/', '', $nama_mp)).$kode_jurusan.$kode_ta;
			if ($stmt->execute()) {
				$result = $stmt->get_result();
				while ($i = $result->fetch_array()) {
					if (strtolower(preg_replace('/\s/', '', $i['nama_mp'])).$i['kode_jurusan'].$i['kode_ta']==$gabung) {
						return 1;
						break;
					}
				}
